           ...
           (test-section "run-exporter in foreground (blocking)"
                         (let ((endpoint "http://localhost:4318/v1/traces")
                           ...
                           (flet ((mock-post (url &key headers content &allow-other-keys)
                                    ...
                                    (return-from test-run-exporter-foreground (is string= data-sent "test-span-data" "data was sent to exporter"))))
...
(test-case test-create-resource
           (test-section "create-resource function"
                         (let ((resource-attrs `((:service.name "test-service") (:service.version "1.0")))
                           ...
                           (is typep resource-spans 'otel.trace:resource-spans "create-resource returns resource-spans")
                           (is typep (otel.trace:resource resource-spans) 'otel.trace:resource "resource-spans contains a resource")
